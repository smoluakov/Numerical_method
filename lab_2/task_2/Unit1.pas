...
function Spline(x1:real):real;      //Ïîñòğîåíèå èíòåğïîëÿöèîííîãî êóáè÷åñêîãî ñïëàéíà
var i:integer;
    ...
    s:real;
begin
  for i := 1 to n do                             //Ïğÿìîé õîä ìåòîäà ïğîãîíêè
    h[i]:=x[i]-x[i-1];
  for i := 2 to n do
    begin
      w[i]:=h[i-1];
  ...
  c[n+1]:=0;                                    //Îáğàòíûé õîä ìåòîäà ïğîãîíêè
  for i := n downto 1 do
    c[i]:=k[i]-l[i]*c[i+1];

  for i := 1 to n do                            //Ïîñòğîåíèå ñïëàéíà
...
procedure TForm1.Button1Click(Sender: TObject);   //Èñõîäíàÿ ôóíêöèÿ
var i:integer;
begin
  for i := 0 to n do
...
procedure TForm1.Button2Click(Sender: TObject);  //Âû÷èñëåíèå çíà÷åíèé ôóíêöèè â òî÷êàõ x=1.13 è x=1.17
begin
